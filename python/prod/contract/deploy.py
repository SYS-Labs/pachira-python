...
class Deploy():
    ...
    def deploy_contract(
        ...
        *constructor_args,
        ...
        if register_for_tracing:
            ...
            self.register_contract(web3, tx_receipt["contractAddress"], instance)
    
        return instance


    def register_contract(self, web3, address: HexAddress, instance: Contract):
        """Register a contract for tracing.
    
        See :py:func:`deploy_contract`.
        """
        assert type(address) == str, f"address is {type(address)}, expected str"
        registry = self.get_or_create_contract_registry(web3)
        registry[address.lower()] = instance

    def get_registered_contract(self, web3, address: str) -> Contract:
    ...
    def get_or_create_contract_registry(self, web3: Web3) -> ContractRegistry:
